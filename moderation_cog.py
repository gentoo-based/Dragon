...
class ModerationCog(commands.Cog):
    ...
    @commands.Bot.slash_command(self, name="clear", description="Clears a specific number of messages from a channel")
    @commands.has_permissions(manage_messages=True)
    ...
    @commands.Bot.slash_command(self, name="ban", description="Bans a member from the server")
    @commands.has_permissions(ban_members=True)
    async def slash_ban(self, ctx: nextcord.Interaction, member: nextcord.Member, *, reason=None):
        await member.ban(reason=reason)
        await ctx.send(f'User {member} has been banned')

    @commands.Bot.slash_command(self, name="kick", description="Kicks a member from the server")
    @commands.has_permissions(kick_members=True)
    async def slash_kick(self, ctx: nextcord.Interaction, member: nextcord.Member, *, reason=None):
        await member.kick(reason=reason)
        await ctx.send(f'User {member} has been kicked')

    @commands.Bot.slash_command(self, name="timeout", description="Times out a member for a specified number of seconds")
    @commands.has_permissions(manage_messages=True)
    ...
    @commands.Bot.slash_command(self, name="lock", description="Locks the current channel")
    @commands.has_permissions(manage_channels=True)
    ...
    @commands.Bot.slash_command(self, name="warn", description="Warns a user. 3 warnings = kick, 6 warnings = ban")
    @commands.has_permissions(kick_members=True, ban_members=True)
